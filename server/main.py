...
if (os.path.exists(video_clips_json_path)):
    ...
    if (len(video_clips_to_concat)) > 0:
        concatenated_video = concatenate_videoclips(video_clips_to_concat, method="chain").set_position(("center", "center"))
...
if (concatenated_video != None):
    ...
    if (composite_audio != None):
        ...
        if (final_video.audio != None):
            final_audio = CompositeAudioClip([final_audio.audio, composite_audio])

        final_video.audio = final_audio
        final_video.write_videofile(ready_to_stream_file_path, audio_codec="aac", threads=NUM_THREADS, preset=PRESET)
        video_created = True
    else:
        final_video = CompositeVideoClip([concatenated_video], size=video_size)
        final_video.write_videofile(ready_to_stream_file_path, audio_codec="aac", threads=NUM_THREADS, preset=PRESET)
